...
public class Boots extends Item {
    ...
    public Boots(int x, int y, ItemRarity rarity) {
        super(x, y);
        tier = 1 + (int) Math.ceil(Level.worldLevel / 10);
        itemRarity = rarity;
        dropSprite = Sprite.boots;
    ...
    protected void setSprite() {
        switch (itemRarity) {
            case NORMAL:
                sprite = Sprite.boots_normal;
                icon = Sprite.boots_icon_normal;
                break;
            case MAGIC:
                sprite = Sprite.boots_magic;
                icon = Sprite.boots_icon_magic;
                break;
            case RARE:
                sprite = Sprite.boots_rare;
                icon = Sprite.boots_icon_rare;
                break;
            case UNIQUE:
                sprite = Sprite.boots_unique;
                icon = Sprite.boots_icon_unique;
                break;
            default:
                sprite = Sprite.boots;
        }
    ...
    public int[] setItemStats(int[] stats) {
        ...
        switch (itemRarity) {
            ...
            case RARE:
                ...
                // THIRD STAT
                if (thirdStat == 0) {
                    statValue[2] = movementSpeed;
                    stats[4] = movementSpeed;
                    stats[5] = 0;
                    statName[2] = "M.SPD ";
                }
                if (thirdStat == 1) {
    ...
    public int[] getStats(int[] stats) {
        stats[30] = this.stats[0];
        stats[31] = this.stats[1];
        stats[32] = this.stats[2];
        stats[33] = this.stats[3];
        stats[34] = this.stats[4];
        stats[35] = this.stats[5];
        return stats;
    }
